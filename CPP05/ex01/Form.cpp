...
/*default constructor*/
Form::Form() : _name("Default"), _signed(false), _gradeSign(0), _gradeExecute(0)
{
...
Form::Form(std::string name, int gradeSign, int gradeExecute) : _name(name),
	_signed(false), _gradeSign(gradeSign), _gradeExecute(gradeExecute)
{
...
Form::Form(const Form &rhs) : _name(rhs._name), _signed(rhs._signed),
	_gradeSign(rhs._gradeSign), _gradeExecute(rhs._gradeExecute)
{
...
{
	return ("Form sign/execute Grade is too high!");
}

const char *Form::GradeTooLowException::what() const throw()
{
	return ("Form sign/execute Grade is too low!");
}
