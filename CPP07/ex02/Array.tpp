...
template <typename T>
Array<T>::Array(unsigned int n) : element(new T[n]()), _size(n) {}

/* Copy constructor to create a deep copy of another Array */
template <typename T>
Array<T>::Array(const Array& rhs) : element(NULL), _size(rhs._size) {
    if (_size > 0) {
        element = new T[_size];
...
/* Copy assignment operator to assign one Array to another */
template <typename T>
Array<T>& Array<T>::operator=(const Array& rhs) {
    if (this != &rhs) { // Avoid self-assignment
        ...
        _size = rhs._size;
        element = (_size > 0) ? new T[_size] : NULL;
        for (unsigned int i = 0; i < _size; ++i) {
            element[i] = rhs.element[i];
        }
    }
