...
template <typename T> void fillContainer(T &t1, int startValue)
{
...
int	main(void)
{
	...
	try
	{
		...
		std::cout << "Value 5 found in vector at position: " << std::distance(myVec.begin(),
			result) << std::endl;
	}
	...
	try
	{
		...
		std::cout << "Value 8 found in list at position: " << std::distance(myList.begin(),
			result) << std::endl;
	}
	catch (const std::exception &e)
	{
	...
	try
	{
		...
		std::cout << "Value 11 found in deque at position: " << std::distance(myDeque.begin(),
			result) << std::endl;
	}
	catch (const std::exception &e)
	{
